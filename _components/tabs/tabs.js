...
import { q, closest } from 'frend-utils'
...
const Frtabs = function ({
  ...
  let tabContainers = q(selector);


  // A11Y
  function _addA11y (tabContainer) {
    // get tab elements
    const tabLists = q(tablistSelector, tabContainer);
    const tabListItems = q(tablistSelector + ' li', tabContainer);
    const tabs = q(tablistSelector + ' a', tabContainer);
    const tabpanels = q(tabpanelSelector, tabContainer);
  ...
  function _showTab (target, giveFocus = true) {
    ...
    let thisContainer = closest(target, selector);
    let siblingTabs = q(tablistSelector + ' a', thisContainer);
    let siblingTabpanels = q(tabpanelSelector, thisContainer);
  ...
  function _eventTabKeydown (e) {
    ...
    let tablist = closest(currentTab, tablistSelector);
    ...
    switch (e.keyCode) {
      case 37:
      case 38:
        _showTab(q('[role="tab"]', previousTabItem)[0]);
        e.preventDefault();
        break;
      case 39:
      case 40:
        _showTab(q('[role="tab"]', nextTabItem)[0]);
        e.preventDefault();
        break;
      default:
        break;
  ...
  function init () {
    if (tabContainers.length) {
      tabContainers.forEach((tabContainer) => {
        ...
        _showTab(q(tablistSelector + ' a', tabContainer)[0], false);
